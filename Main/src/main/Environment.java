...
class Environment extends Node {
    ...
    private static Node find(Node id, Node alist) {
        if (!alist.isPair()) {
            ...
            if (id.getName().equals(bind.getCar().getName())) // return a list containing the value as only element
            {
                return bind.getCdr();
            } else {
    ...
    public Node lookup(Node id) {
        ...
        if (val == null && env == null) {
            ...
            return null;
        } else if (val == null) // look up the identifier in the enclosing scope
        {
            return env.lookup(id);
        } else // get the value out of the list we got from find()
        {
            return val.getCar();
        }
    }
    ...
    public void assign(Node id, Node val) {
        ...
        if (oldVal != null) {
            ...
            System.err.print("Cannot assign a value to a nonexistent identifier.");
        }
    }
}
